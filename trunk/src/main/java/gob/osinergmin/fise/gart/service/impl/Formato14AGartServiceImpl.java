...
public class Formato14AGartServiceImpl implements Formato14AGartService {
		...
		try {
			...
			if(existe){
				...
				try{
					...
					throw new Exception("Se produjo un error al guardar los datos del Formato 14A");
				}
			}
			//add
	...
	public FiseFormato14AC modificarFormato14AC(Formato14ACBean formulario, FiseFormato14AC fiseFormato14AC) throws Exception {
		...
		try {
			...
			logger.error("--error"+e.getMessage());
			e.printStackTrace();
			throw new Exception("Se produjo un error al actualizar los datos del Formato 14A");
		}
		//
		return dto;
	...
	public void eliminarFormato14AC(FiseFormato14AC fiseFormato14AC) {
		...
		String valor = eliminarArchivoSustentoCab(fiseFormato14AC);
		logger.info("Valor al eliminar archivos de sustento si es 1= OK caso contrario error:  "+valor); 
		
	}
	
	//@Transactional
	private String eliminarArchivoSustentoCab(FiseFormato14AC f){
		String valor = "0";
		...
		try {
			...
			listaCab = archivoSustentoDao.listaFiseArchivosCabBienal(f.getId().getCodEmpresa(),
					...
					anioInicioVig, anioFinVig, f.getId().getEtapa(),FiseConstants.NOMBRE_FORMATO_14A);
